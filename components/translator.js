      ...
      case "american-to-british":
        textArr.forEach((word, i) => {
          if (Object.keys(americanToBritishSpelling).includes(word)) {
            let replacementWord = Object.values(americanToBritishSpelling).find(
              (value) => americanToBritishSpelling[word] === value
            );
            textArr[i] = `<span class="highlight">${replacementWord}</span>`;
          }
          if (Object.keys(americanOnly).includes(word)) {
            let replacementWord = Object.values(americanOnly).find(
              (value) => americanOnly[word] === value
            );
            textArr[i] = `<span class="highlight">${replacementWord}</span>`;
          }
        });
      ...
      case "british-to-american":
        textArr.forEach((word, i) => {
          ...
          if (Object.keys(britishOnly).includes(word)) {
            let replacementWord = Object.values(britishOnly).find(
              (value) => britishOnly[word] === value
            );
            textArr[i] = `<span class="highlight">${replacementWord}</span>`;
          }
        });
  ...
  checkForTitles(textArr, locale) {
    switch (locale) {
      case "american-to-british":
        textArr.forEach((word, i) => {
          if (
            Object.keys(americanToBritishTitles).includes(word.toLowerCase())
          ) {
      ...
      case "british-to-american":
        textArr.forEach((word, i) => {
          if (
            Object.values(americanToBritishTitles).includes(word.toLowerCase())
          ) {
